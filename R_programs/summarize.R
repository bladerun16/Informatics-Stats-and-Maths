...
#### test program ####
province_f_url = "http://www.mandile.it/wp-content/uploads/provincia-regione-sigla.csv"
# take a moment to inspect the file and understand:
# - if there is header
# - which is the separator
# - if there are anomalies
prov_data <- read.csv(url(province_f_url)
, stringsAsFactors = F
, sep = ","
, col.names = c("province", "region", "abbrv")
, header = F
)
summaryTable <- summarize(prov_data$region)
print(summaryTable)
